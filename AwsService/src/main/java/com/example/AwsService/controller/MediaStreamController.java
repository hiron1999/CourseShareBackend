...
public class MediaStreamController {
	...
	@GetMapping(value = "/{resourceMappingId}", produces = "video/mp4")
	public Mono<ResponseEntity<byte[]>> getVedio(@PathVariable String resourceMappingId, @RequestHeader("Range") String range) {
//		System.out.println("renge in header :" + range.toString());
//		return streamService.getVideo(title,range);
        log.info("renge in header :{}", range);
		return streamService.getVideo(resourceMappingId, range);
